...
//OELD滑动模块 
//dir 方向 ；速度 spd ；显示内容  位置*   u8 pos 0 1 2 3 行 对应01 23 45 67页***
void OLED_Roll(u8 dir, u8 spd, u8 pos)
{
	u8 i,d,n;
	unsigned dat=0;
	if(dir==1)
	{
		if(i<100)
		{
			for(d=0;d<2;d++)
			{
				OLED_WR_Byte (0xb0+pos*2+d,OLED_CMD);    //设置页地址（0~7）
				OLED_WR_Byte (0x00,OLED_CMD);      //设置显示位置—列低地址
...
//			OLED_ShowCHinese(spd*i+60,pos*2,(8));
	...
	else
	{
		if(i<100)
		{
			...
			/*-------滑动模块-----------*/
			OLED_ShowCHinese(68-spd*i,pos*2,3);//滑动实验模组
